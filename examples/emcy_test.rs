...
async fn main() -> Result<()> {
    ...
    // Handler for Node 1
    {
        ...
        canopen.register_emcy_handler(1, move |emcy| {
    ...
    // Handler for Node 2
    {
        ...
        canopen.register_emcy_handler(2, move |emcy| {
            count.fetch_add(1, Ordering::SeqCst);
            println!("  [Node 2 EMCY] Error Code: 0x{:04X} - {}", 
                emcy.error_code, emcy.error_code_description());
        }).await?;
    ...
    // Handler for Node 9
    {
        ...
        canopen.register_emcy_handler(9, move |emcy| {
            count.fetch_add(1, Ordering::SeqCst);
            if emcy.is_error_reset() {
                println!("  [Node 9 EMCY] ✓ Error Reset (0x0000)");
            } else {
                println!("  [Node 9 EMCY] Error: 0x{:04X} - {}", 
                    emcy.error_code, emcy.error_code_description());
            }
        }).await?;
    ...
    while start_time.elapsed() < Duration::from_secs(30) {
        ...
        if last_report.elapsed() >= Duration::from_secs(5) {
            ...
            println!("  [{}s] Monitoring... (Node 1: {}, Node 2: {}, Node 9: {})",
                elapsed,
    ...
    println!("  Node 1: {} emergencies", node1_emcy_count.load(Ordering::SeqCst));
    println!("  Node 2: {} emergencies", node2_emcy_count.load(Ordering::SeqCst));
    println!("  Node 9: {} emergencies", node9_emcy_count.load(Ordering::SeqCst));
