use libcanopen_simple::{CANopenSimple, BusSpeed};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let hardware = Box::new(PeakCanAdapter::new(PcanHandle::PcanUsbbus1, BusSpeed::Baud1M));
    ...
    canopen.register_pdo_handler(0x181, move |data| {
        node1_count_clone.fetch_add(1, Ordering::Relaxed);
        println!("  [Node 1] TPDO1 received: {} bytes: {:02X?}", data.len(), data);
    }).await;
    
    let node2_count_clone = Arc::clone(&node2_count);
    canopen.register_pdo_handler(0x182, move |data| {
        node2_count_clone.fetch_add(1, Ordering::Relaxed);
        println!("  [Node 2] TPDO1 received: {} bytes: {:02X?}", data.len(), data);
    }).await;
    ...
    canopen.register_pdo_handler(0x189, |data| {
    ...
    for (idx, &node_id) in nodes_to_monitor.iter().enumerate() {
        ...
        canopen.register_pdo_handler(cob_id, move |data| {
