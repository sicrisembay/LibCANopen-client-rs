...
use crate::canopen::message::{CanMessage, CanId};
...
impl NmtManager {
        ...
        if let Some(node_state) = self.nodes.get_mut(&node_id) {
            ...
            if state_changed {
                log::info!("Node {} state changed: {} -> {}", 
                    node_id, node_state.last_state, node_state.state);
            }
    ...
    pub async fn start_node(&self, node_id: u8) -> Result<()> {
        self.send_nmt_command(NmtCommand::StartRemoteNode, node_id).await
    }
    ...
    pub async fn stop_node(&self, node_id: u8) -> Result<()> {
        self.send_nmt_command(NmtCommand::StopRemoteNode, node_id).await
    }
    ...
    pub async fn enter_pre_operational(&self, node_id: u8) -> Result<()> {
        self.send_nmt_command(NmtCommand::EnterPreOperational, node_id).await
    }
    ...
    pub async fn reset_communication(&self, node_id: u8) -> Result<()> {
        self.send_nmt_command(NmtCommand::ResetCommunication, node_id).await
    }
