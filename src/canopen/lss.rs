...
impl LssManager {
    ...
    pub async fn process_response(&self, data: &[u8]) {
        ...
        if let Some(tx) = response_tx {
            ...
            trace!("Received LSS response with no pending request: CS=0x{:02X}", cs);
        }
    }
    ...
    pub async fn switch_state_global(&self, mode: LssMode) -> Result<()> {
        ...
        let lss_msg = CanMessage::new(LSS_MASTER_TX, command)
            .expect("LSS message creation should never fail");
        self.message_tx.send(lss_msg).await
            .map_err(|_| CANopenError::ChannelClosed)?;
    ...
    pub async fn switch_state_selective(
        &self,
        address: &LssAddress,
    ) -> Result<()> {
    ...
    /// Configure node-ID
    pub async fn configure_node_id(
        &self,
        node_id: u8,
        timeout_ms: u32,
    ) -> Result<LssError> {
        if node_id == 0 || node_id > 127 {
            return Err(CANopenError::InvalidData("Node ID must be 1-127".to_string()));
        }
    ...
    pub async fn activate_bit_rate(&self, switch_delay_ms: u16) -> Result<()> {
        ...
        debug!("LSS: Activated bit-rate (switch delay: {}ms)", switch_delay_ms);
        Ok(())
    }
    ...
    pub async fn inquire_lss_address(&self, timeout_ms: u32) -> Result<LssAddress> {
        // Inquire vendor ID
        let vendor_id = match self.send_command(
        ...
        let product_code = match self.send_command(
        ...
        let revision_number = match self.send_command(
        ...
        let serial_number = match self.send_command(
...
mod tests {
    ...
    fn test_lss_command_from_u8() {
        // Switch Mode Commands
        assert_eq!(LssCommand::from_u8(0x04), Some(LssCommand::SwitchStateGlobal));
        assert_eq!(LssCommand::from_u8(0x40), Some(LssCommand::SwitchStateSelective));
        
        // Configuration Commands
        assert_eq!(LssCommand::from_u8(0x11), Some(LssCommand::ConfigureNodeId));
        assert_eq!(LssCommand::from_u8(0x13), Some(LssCommand::ConfigureBitTiming));
        assert_eq!(LssCommand::from_u8(0x15), Some(LssCommand::ActivateBitTiming));
        assert_eq!(LssCommand::from_u8(0x17), Some(LssCommand::StoreConfiguration));
        
        // Inquire Commands
        assert_eq!(LssCommand::from_u8(0x5A), Some(LssCommand::InquireVendorId));
        assert_eq!(LssCommand::from_u8(0x5B), Some(LssCommand::InquireProductCode));
        assert_eq!(LssCommand::from_u8(0x5C), Some(LssCommand::InquireRevisionNumber));
        assert_eq!(LssCommand::from_u8(0x5D), Some(LssCommand::InquireSerialNumber));
        assert_eq!(LssCommand::from_u8(0x5E), Some(LssCommand::InquireNodeId));
        
        // Identification Commands
        assert_eq!(LssCommand::from_u8(0x46), Some(LssCommand::IdentifyRemoteSlave));
        assert_eq!(LssCommand::from_u8(0x50), Some(LssCommand::IdentifyNonConfiguredSlave));
    ...
    fn test_lss_error_description() {
        ...
        assert_eq!(LssError::UnsupportedCommand.description(), "Unsupported Command");
        assert_eq!(LssError::MediaAccessFailure.description(), "Media Access Failure");
        assert_eq!(LssError::InvalidParameter.description(), "Invalid Parameter");
        assert_eq!(LssError::ManufacturerSpecific.description(), "Manufacturer Specific Error");
    }
    ...
    fn test_lss_message_format() {
        ...
        let master_msg = CanMessage::new(0x7E5, vec![0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]).unwrap();
        assert_eq!(master_msg.id.raw(), 0x7E5);
        assert_eq!(master_msg.data.len(), 8);
        
        // Slave to Master message (COB-ID 0x7E4)
        let slave_msg = CanMessage::new(0x7E4, vec![0x5A, 0x78, 0x56, 0x34, 0x12, 0x00, 0x00, 0x00]).unwrap();
    ...
    fn test_lss_configure_node_id_response() {
        ...
        let data = vec![0x11, LssError::Success as u8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
        let msg = CanMessage::new(LSS_SLAVE_TX, data).unwrap();
    ...
    fn test_lss_switch_state_selective() {
        ...
        let extracted_vendor = u32::from_le_bytes([
            msg.data[1], msg.data[2], msg.data[3], msg.data[4]
        ]);
