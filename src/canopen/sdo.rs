...
impl SdoClient {
    ...
    pub fn new(
        message_sender: mpsc::Sender<CanMessage>,
    ) -> (Self, mpsc::Sender<SdoRequest>, mpsc::Sender<CanMessage>) {
    ...
    pub async fn process_message(&mut self, message: &CanMessage) -> Result<()> {
        ...
        if let Some(mut transfer) = self.active_transfers.remove(&node_id) {
                ...
                match (transfer.direction, transfer.state) {
                    (SdoDirection::Upload, SdoState::InitiateSent) => {
                        if command & 0x43 == 0x43 {
                            ...
                            self.send_upload_segment_request(transfer.node_id, transfer.toggle_bit)
                                .await?;
                        }
                    }
                    (SdoDirection::Upload, SdoState::SegmentTransfer) => {
                        ...
                        if command & 0xE0 == 0x00 {
                            ...
                            if toggle == transfer.toggle_bit {
                                ...
                                transfer
                                    .data_buffer
                                    .extend_from_slice(&message.data[1..1 + data_length]);
                                ...
                                if c {
                                    ...
                                    self.send_upload_segment_request(
                                        transfer.node_id,
                                        transfer.toggle_bit,
                                    )
                                    .await?;
                                ...
                                if let Some(sender) = transfer.response_sender.take() {
                                    let _ =
                                        sender.send(Err(CANopenError::Sdo { code: 0x05030000 }));
                                }
                                transfer.state = SdoState::Aborted;
                                should_complete = true;
                            }
                    ...
                    (SdoDirection::Download, SdoState::SegmentTransfer) => {
                        ...
                        if command & 0xE0 == 0x20 {
                            ...
                            if toggle == transfer.toggle_bit {
                                ...
                                if (transfer.segment_index as usize) * 7
                                    >= transfer.data_buffer.len()
                                {
                                    // All segments sent
                                    if let Some(sender) = transfer.response_sender.take() {
                                        let _ = sender.send(Ok(vec![]));
            ...
            log::warn!(
                "Received SDO response from node {} with no active transfer",
                node_id
            );
        }
    ...
    async fn handle_sdo_request(&mut self, request: SdoRequest) -> Result<()> {
        ...
        if self.active_transfers.contains_key(&node_id) {
            let _ = request
                .response_sender
                .send(Err(CANopenError::Sdo { code: 0x05040001 }));
            return Ok(());
        }
        ...
        match request.direction {
            SdoDirection::Upload => {
                self.send_upload_initiate(node_id, request.index, request.subindex)
                    .await?;
            }
            SdoDirection::Download => {
                if request.data.len() <= 4 {
                    self.send_download_initiate_expedited(
                        node_id,
                        request.index,
                        request.subindex,
                        &request.data,
                    ...
                    self.send_download_initiate_segmented(
                        node_id,
                        request.index,
                        request.subindex,
                        request.data.len(),
        ...
        log::debug!(
            "Started SDO {} for node {}, index 0x{:04X}:{:02X}",
            match request.direction {
    ...
    async fn send_download_initiate_expedited(
        &self,
        node_id: u8,
        index: u16,
        subindex: u8,
    ...
    async fn send_download_initiate_segmented(
        &self,
        node_id: u8,
        index: u16,
        subindex: u8,
    ...
    async fn send_download_segment(&self, transfer: &SdoTransfer) -> Result<()> {
        ...
        // Copy segment data
        for (i, &byte) in transfer.data_buffer[start_byte..start_byte + bytes_to_send]
            .iter()
            .enumerate()
        {
            data[1 + i] = byte;
...
impl SdoClient {
    ...
    pub async fn read_u8(
        &self,
        sdo_sender: &mpsc::Sender<SdoRequest>,
        node_id: u8,
        index: u16,
        subindex: u8,
    ) -> Result<u8> {
    ...
    pub async fn write_u8(
        ...
        self.write_data(sdo_sender, node_id, index, subindex, vec![value])
            .await
    }
    ...
    pub async fn write_u16(
        ...
        self.write_data(sdo_sender, node_id, index, subindex, data)
            .await
    }
    ...
    pub async fn read_data(
        ...
        timeout(Duration::from_millis(1500), response_receiver)
            .await
